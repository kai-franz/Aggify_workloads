...
--UDF call
select C_CUSTKEY, dbo.OrdersByCustomer(C_CUSTKEY)
from customer
       ...
       (select dt4.retval
        from (
                     ...
                     cross apply
                         (select 0 as val) dt1
                     cross apply
                     (select dbo.OrdersByCustomer1(dt2.O_ORDERKEY, 0) as udtVal
                      from (select O_ORDERKEY from orders where O_CUSTKEY = dt0.ckey) dt2) dt3
                     cross apply
